...
За 6-8 годин потрібно виконати короткий modern-frontend review, додати дві
сторінки (`/requests` і `/requests/:requestId`), підключити готовий typed mock
API через TanStack Query, реалізувати безпечний confirmation/pending/result UX
для зовнішньої signing session і щонайменше чотири React UX tests. Не потрібно
змінювати backend, писати mock API, реалізовувати справжню КЕП або будувати нову
design system.
...
Додати у наявний прототип портал `Requests` hub із release-ready вертикальним
сценарієм та показати сучасний підхід до React:
...
## Обов'язкова частина 0: Modern frontend review

Витратьте на цю частину не більше 45-60 хвилин усередині загального timebox.
До початку основної реалізації створіть `MODERN-FRONTEND-REVIEW.md`, максимум
одна сторінка.

Розгляньте навмисно проблемний приклад:
...
export function RequestsPage() {
  ...
  useEffect(() => {
    setFiltered(
      requests.filter((request) => request.name.includes(query))
    );
  }, [requests, query]);
  ...
  const sign = async (requestId: string) => {
    ...
    await fetch(`/api/requests/${requestId}/sign`, { method: 'POST' });
  };
  ...
  return (
    <div style={{ color: '#555' }}>
      ...
      {filtered.map((request) => (
        <div key={request.id} onClick={() => sign(request.id)}>
...
1. знайти щонайменше 10 конкретних проблем або ризиків;
2. розподілити їх за категоріями `Modern React`, `Frontend architecture`,
   `UX/accessibility`, `API/mock correctness`;
3. пояснити, що має бути derived під час render, що належить TanStack Query, а
   що залишається local UI state;
4. намалювати невелику Mermaid-схему залежностей між page/module, query hooks,
   API adapter, `business-ui`, `shared-ui` та design tokens;
5. назвати п'ять UX-станів, без яких цей flow не можна випускати;
6. коротко описати, чому optimistic `Submitted` тут небезпечний;
7. визначити, які дві речі ви свідомо не будете абстрагувати у першому релізі.

Не потрібно окремо переписувати цей snippet. Основна реалізація нижче має
показати виправлений підхід на практиці.
...
- `docs/architecture/FITNESS_FUNCTIONS.md`.

Не переписуйте shell та існуючі hubs. Додайте новий вертикальний сценарій у
поточну архітектуру.
...
Додайте route `/requests` і пункт `Requests` у навігацію.
...
- [ ] `python scripts/check_architecture.py` проходить;
...
- micro-frontends;
- повний redesign існуючого portal shell;
